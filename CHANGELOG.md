...
### Added
- **Cumulative token badge on context-bar.** The full-width bar now shows a `24k` (always-rounded K, no decimals) dim-grey badge at the right edge — cumulative `total_input_tokens + total_output_tokens` for the session. Toggle via `show.contextTokens`. Silent before the first API call.
- **CC update notification.** When a newer `@anthropic-ai/claude-code` is published on npm, a `update avail 2.x.y` notice appears on the context-bar row to the left of the token badge. Check runs at most once per hour in a detached background process — never blocks the render. Opt out via `show.ccUpdate: false` or `LEAN_STATUSLINE_NO_NETWORK=1`.
- **`subagentStatusLine` renderer** (`lean-statusline-subagents` bin). A second entry point that CC invokes per-subagent to replace the default row. Shows: status icon (●/✓/✗/⏸) + name + token count (Nk tok) + elapsed time. Wire via `subagentStatusLine: { type: "command", command: "lean-statusline-subagents" }` in `~/.claude/settings.json`, or the install flow will offer it. Respects your palette, barStyle, and icons settings.

### Changed
- **Context % now uses `remaining_percentage` as primary source.** `100 - context_window.remaining_percentage` is Anthropic's own calculation of what's left — it's what CC itself uses to decide when to compact. No multipliers or guesswork. Falls back to `used_percentage`, then local math from `current_usage` for old CC builds that don't ship either field.
