...
// Pure function — no IO, no ambient clock. Caller supplies task + palette +
// icons + now (ms since epoch); pass `Date.now()` from the bin entry point
// or any fixed value from tests.
...
export function renderTask(task, palette, icons, now = Date.now()) {
    ...
    const elapsed = startTime ? fmtElapsed(now - startTime) : null;
