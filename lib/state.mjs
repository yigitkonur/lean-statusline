...
function normalizeState(parsed, now = Date.now()) {
    const fresh = freshState(now);
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return fresh;
    if (parsed.schemaVersion !== SCHEMA_VERSION) return fresh;

    const counters = asObject(parsed.counters);
    return {
        schemaVersion: SCHEMA_VERSION,
        lastSeen: { ...asObject(parsed.lastSeen) },
        counters: {
            messagesSinceLastChange: { ...asObject(counters.messagesSinceLastChange) },
            totalRenders: asNonNegativeNumber(counters.totalRenders, fresh.counters.totalRenders),
            firstSeenAt: asNonNegativeNumber(counters.firstSeenAt, fresh.counters.firstSeenAt),
            lastRenderAt: asNonNegativeNumber(counters.lastRenderAt, fresh.counters.lastRenderAt),
        },
    };
}
