...
export const DEFAULTS = Object.freeze({
    ...
    show: {
        ...
        // 1.5.3: off by default. Previously auto-elevated worktree names
        // before dir whenever worktree data was present; users found it
        // noisy in the wizard preview since sample data always provides
        // one. Opt in via wizard page 4 (core segments).
        worktree: false,
        paceDelta: true,
        subagents: true,
...
// Stamped onto saved configs so one-shot migrations run at most once per
// user. Bump when new defaults roll over: existing configs whose stamp is
// older get the listed fields reset to the new defaults below, assuming
// they're using a recognized preset (i.e. took the preset's defaults).
const CONFIG_VERSION = '1.5.3';
const KNOWN_PRESET_NAMES = ['minimal', 'compact', 'full', 'classic'];
...
function normalizeConfig(cfg) {
    ...
    // 1.5.3: default-off migrations for preset-based configs. Trigger only
    // when the user is on a known preset AND `ssh` is still listed in their
    // segments — that's a strong signal the config predates 1.5.3's ssh-off
    // default (all presets used to ship ssh first). In that case, reset the
    // three fields that changed their default in 1.5.3: strip ssh, flip
    // `show.worktree`/`show.branch` to false. Users who removed ssh but kept
    // show.worktree=true explicitly are left alone — the linkage avoids
    // clobbering intentional choices. Stamp the version so this runs once.
    if (normalized._configVersion !== CONFIG_VERSION
        && KNOWN_PRESET_NAMES.includes(normalized.preset)
        && Array.isArray(normalized.segments)
        && normalized.segments.includes('ssh')) {
        normalized.segments = normalized.segments.filter(s => s !== 'ssh');
        warnings.push(`1.5.3: removed "ssh" from preset segments — opt back in via wizard page 4 (core segments).`);
        if (normalized.show && typeof normalized.show === 'object') {
            if (normalized.show.worktree === true) {
                normalized.show.worktree = false;
                warnings.push(`1.5.3: show.worktree now defaults to false — opt back in via wizard page 4.`);
            }
            if (normalized.show.branch === true) {
                normalized.show.branch = false;
                warnings.push(`1.5.3: show.branch now defaults to false — opt back in via wizard page 4.`);
            }
        }
    }
    if (normalized._configVersion !== CONFIG_VERSION) {
        normalized._configVersion = CONFIG_VERSION;
    }
