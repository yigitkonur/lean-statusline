...
// Directory: basename / tilde-home / full.
function formatDir(cwd, style) {
    if (!cwd) return cwd;
    if (style === 'basename') return cwd.split(/[/\\]/).filter(Boolean).pop() || cwd;
    if (style === 'tilde') {
        const h = homedir();
        if (h && cwd === h) return '~';
...
function renderRateFull(label, lim, ctx, barWidth) {
