...
// Priority: remaining_percentage (Anthropic's own remaining calc, most accurate) →
//           used_percentage (pre-calculated) → local math from current_usage.
    ...
    if (lim.resetsAt) {
        ...
        if (cd) {
            if (showEta && Number.isFinite(metrics.etaMs)) {
                ...
                const critMs = ctx.cfg.pace?.etaCriticalMs ?? 3_600_000;
                const color = metrics.etaMs < critMs ? ctx.palette.red : ctx.palette.orange;
...
function renderPaceExplainer(ctx) {
    ...
    // Preview: synthesize a prior ETA 30 min larger so users see the 'slower' copy.
    const lastEtaMs = ctx.pacePreview && prevEtaMs == null ? metrics.etaMs + 30 * 60_000 : prevEtaMs;
    ...
    const critMs = ctx.cfg.pace?.etaCriticalMs ?? 3_600_000;
    const accentColor = metrics.etaMs < critMs ? ctx.palette.red : ctx.palette.orange;
    const iconColor = ctx.palette.orange;
...
export const SEGMENTS = {
    ...
    'context-bar'(ctx) {
        ...
        // Update notice (shown when a newer CC version is available).
        if (ctx.ccUpdate && ctx.cfg.show.ccUpdate !== false) {
            out += ` ${ctx.palette.dim('update avail')} ${ctx.palette.yellow(ctx.ccUpdate)}`;
        }
        // Current context-size badge — matches `/context` output exactly.
        // Always visible when we have any signal from CC (non-zero).
        if (ctx.cfg.show.contextTokens !== false) {
            const totalTok = computeContextTokens(ctx.input);
            if (totalTok > 0) out += ` ${ctx.palette.dim(fmtKTokens(totalTok))}`;
        }
        return out;
    },
