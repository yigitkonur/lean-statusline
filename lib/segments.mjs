...
function fmtPct(pct, ctx, barWidth) {
    ...
    // 1.5.0: `show.bars` master toggle removed. Bar always renders — users
    // set the width knob to 0 if they want a numbers-only readout.
    if (w > 0) {
...
// Format a token count as Nk (rounded to nearest thousand, no decimals).
function fmtKTokens(n) {
    return `${Math.round(n / 1000)}k`;
}

// Current context size in tokens — matches the number shown by `/context`.
//
// Priority:
//   1. used_percentage × context_window_size  — same signal `/context` prints
//      ("24.3k/1m tokens (2%)"). Includes system prompt, tools, memory, skills,
//      custom agents, and conversation messages. Reads the RAW float so we don't
//      lose precision to probe-level rounding (2.43% → 2% would drop ~4k).
//   2. (100 − remaining_percentage) × context_window_size — same idea from the
//      free-space side; may slightly over-count (counts the autocompact buffer).
//   3. total_input_tokens + total_output_tokens — per-API-call tokens. These
//      are tiny for short messages (e.g. "." is ~13 tokens → rounds to 0k) and
//      don't reflect the baseline boilerplate (system prompt, tools, skills).
//      Kept only as a last-resort so legacy payloads still render something.
// 3-tier urgency color for a time-until-exhaustion estimate, using the two
// thresholds from cfg.pace: etaCriticalMs (< → red) and etaWarnMs (< → orange),
// else yellow. Only meaningful when the caller has already decided the ETA is
// urgent (i.e. exhaustsBeforeReset).
function colorForEta(etaMs, paceCfg, palette) {
    const critMs = paceCfg?.etaCriticalMs ?? 3_600_000;
    const warnMs = paceCfg?.etaWarnMs ?? 14_400_000;
    if (etaMs < critMs) return palette.red;
    if (etaMs < warnMs) return palette.orange;
    return palette.yellow;
}

function computeContextTokens(input) {
    const size = input?.context_window?.context_window_size || 200000;
    const usedRaw = input?.context_window?.used_percentage;
    if (typeof usedRaw === 'number' && Number.isFinite(usedRaw)) {
        return Math.round(size * usedRaw / 100);
    }
    const remRaw = input?.context_window?.remaining_percentage;
    if (typeof remRaw === 'number' && Number.isFinite(remRaw)) {
        return Math.round(size * (100 - remRaw) / 100);
    }
...
// Priority matches computeContextTokens so the `ctx` header % and the
// context-bar token badge never disagree mathematically:
//   1. used_percentage — matches `/context` exactly (conversation only, no
//      autocompact buffer). Authoritative.
//   2. remaining_percentage — fallback; (100 − this) may slightly over-count
//      because `remaining` is "free space only", excluding autocompact buffer.
//   3. local math from current_usage.
// No multipliers — Anthropic's numbers are authoritative.
export function readContextPct(input) {
    ...
    const rem = probe('context_window.remaining_percentage', input);
    if (rem != null && Number.isFinite(rem)) {
        return Math.min(100, Math.round(100 - rem));
    }
...
function renderPaceArrow(lim, windowKey, ctx) {
    ...
    if (ctx.cfg.pace?.mode === 'off') return '';
    // In ETA mode on the 5h window, renderRateFull handles display via `vs est:`.
    // The inline `5h` segment calls renderPaceArrow directly — suppress the
    // delta there too so the two 5h displays stay consistent.
    if (ctx.cfg.pace?.mode === 'eta' && windowKey === 'five_hour') return '';
...
//
// Bar always renders when width > 0 (1.5.0 dropped the `show.bars` toggle).
// Set `rateBarWidth: 0` for a numbers-only readout.
function renderRateFull(label, lim, ctx, barWidth, windowKey) {
    ...
    // ETA mode (5h only): when burn will hit 100% before reset, replace the
    // `⇡+N` delta with an inline `vs est:` estimate colored by urgency.
    // `LEAN_STATUSLINE_PACE_PREVIEW=1` (or ctx.pacePreview) forces the display
    // on so users can see the format from the configure wizard.
    const mode = ctx.cfg.pace?.mode ?? 'eta';
    const etaActive = windowKey === 'five_hour' && mode === 'eta' && ctx.cfg.show.paceDelta !== false;
    const metrics = etaActive
        ? readPaceDelta(lim.pct, lim.resetsAt, windowKey, Date.now(), ctx.cfg.pace)
        : null;
    const showEta = etaActive && (metrics.exhaustsBeforeReset || ctx.pacePreview);

    let paceArrow = '';
    if (!showEta && mode !== 'off') paceArrow = renderPaceArrow(lim, windowKey, ctx);

    let out = `${tagPart(labelKey, ctx.palette.white(label))}${bar}${tagPart(pctKey, ` ${pctStr}`)}${paceArrow}`;

    if (lim.resetsAt) {
        const cd = fmtCountdown(lim.resetsAt);
        if (cd) {
            if (showEta && Number.isFinite(metrics.etaMs)) {
                const estStr = fmtRemaining(Math.max(0, Math.floor(metrics.etaMs / 1000)));
                const color = colorForEta(metrics.etaMs, ctx.cfg.pace, ctx.palette);
                out += tagPart(countdownKey, ` ${ctx.palette.dim(`(${cd} vs est:`)} ${color(estStr)}${ctx.palette.dim(')')}`);
            } else {
                out += tagPart(countdownKey, ` ${ctx.palette.dim(`(${cd})`)}`);
            }
        }
    }
    return out;
...
// Pace explainer — transient row that teaches what `vs est:` means.
// Shows for 10s when urgency activates, re-appears every 10min while urgency
// persists. Adapts copy to the burn-rate trend (first/steady/faster/slower)
// using the ETA from the previous render, persisted via ctx.state.lastSeen.
//
// Silent when: non-urgent, 5h quota not present, cfg.pace.explainer=false,
// or outside the 10s show window (unless ctx.pacePreview forces it open).
function renderPaceExplainer(ctx) {
    if (ctx.cfg.pace?.explainer === false) return null;
    if (ctx.cfg.pace?.mode === 'off' || ctx.cfg.pace?.mode === 'delta') return null;

    const lim = ctx.rateLimits?.fiveHour;
    if (!lim || lim.pct == null) return null;

    const metrics = readPaceDelta(lim.pct, lim.resetsAt, 'five_hour', Date.now(), ctx.cfg.pace);
    const urgent = metrics.exhaustsBeforeReset || ctx.pacePreview;
    const state = ctx.state?.lastSeen ?? {};
    const prevShown = state.paceExplainerShownAt ?? null;
    const prevEtaMs = state.paceExplainerLastEtaMs ?? null;

    if (!urgent) {
        // Clear state so the next time urgency activates we get a fresh 'first' greeting.
        if (ctx.state?.lastSeen) {
            delete ctx.state.lastSeen.paceExplainerShownAt;
            delete ctx.state.lastSeen.paceExplainerLastEtaMs;
        }
        return null;
    }

    const now = Date.now();
    const durMs = ctx.cfg.pace?.explainerDurationMs ?? 10_000;
    const intMs = ctx.cfg.pace?.explainerIntervalMs ?? 600_000;
    const trendMs = ctx.cfg.pace?.explainerTrendThresholdMs ?? 60_000;

    // Decide whether we're inside the current show window or should open a new one.
    let shownAt = prevShown;
    if (shownAt == null || now - shownAt >= intMs) {
        shownAt = now;
    }
    const inWindow = ctx.pacePreview || (now - shownAt < durMs);

    // Persist state for the next render regardless of whether we render this time.
    // lastEtaMs is always the prior render's ETA so trend compares against the
    // most recent sample (not the sample from the last *shown* render).
    if (ctx.state?.lastSeen) {
        ctx.state.lastSeen.paceExplainerShownAt = shownAt;
        if (Number.isFinite(metrics.etaMs)) {
            ctx.state.lastSeen.paceExplainerLastEtaMs = metrics.etaMs;
        }
    }

    if (!inWindow) return null;
    if (!Number.isFinite(metrics.etaMs)) return null;

    // Preview: synthesize a prior ETA 30 min *smaller* than the current one
    // so the trend classifies as 'slower' (ETA grew → pace eased). This is
    // the most informative demo copy for a wizard preview.
    const lastEtaMs = ctx.pacePreview && prevEtaMs == null ? Math.max(0, metrics.etaMs - 30 * 60_000) : prevEtaMs;

    let trend = 'first';
    if (lastEtaMs != null && Number.isFinite(lastEtaMs)) {
        if (Math.abs(metrics.etaMs - lastEtaMs) < trendMs) trend = 'steady';
        else if (metrics.etaMs < lastEtaMs - trendMs) trend = 'faster';
        else if (metrics.etaMs > lastEtaMs + trendMs) trend = 'slower';
    }

    const etaStr = fmtRemaining(Math.max(0, Math.floor(metrics.etaMs / 1000)));
    const resetStr = fmtRemaining(Math.max(0, Math.floor(metrics.resetInMs / 1000)));
    const prevStr = lastEtaMs != null && Number.isFinite(lastEtaMs)
        ? fmtRemaining(Math.max(0, Math.floor(lastEtaMs / 1000)))
        : null;

    const warnIcon = ctx.icons.ctx === '%' ? '!!' : (ctx.icons.warn ?? '⚠');
    const accentColor = colorForEta(metrics.etaMs, ctx.cfg.pace, ctx.palette);
    // Icon tracks the accent — red icon when critical, orange for warn-tier,
    // yellow for mild (so the explainer's severity matches the inline est).
    const iconColor = accentColor;

    let body;
    switch (trend) {
        case 'faster':
            body = `Pace sped up — 5h cap now in ${accentColor(etaStr)}${ctx.palette.dim(` (was ${prevStr}, resets in ${resetStr})`)}`;
            break;
        case 'slower':
            body = `Pace easing — 5h cap now in ${accentColor(etaStr)}${ctx.palette.dim(` (was ${prevStr}, resets in ${resetStr})`)}`;
            break;
        case 'steady':
            body = `Holding pace — 5h cap still in ${accentColor(etaStr)}${ctx.palette.dim(` (resets in ${resetStr})`)}`;
            break;
        case 'first':
        default:
            body = `At this pace, 5h cap hits in ${accentColor(etaStr)}${ctx.palette.dim(` (resets in ${resetStr})`)}`;
            break;
    }
    return `${iconColor(warnIcon)} ${body}`;
}
...
export const SEGMENTS = {
    ...
    // ctx — always on (locked in the wizard). Label style is user-configurable:
    //   show.ctxLabel = 'text' (default) → `context 32%`
    //   show.ctxLabel = 'icon'           → `✎ 32%` (legacy)
    //   show.ctxLabel = 'none'           → `32%`
    ctx(ctx) {
        const pct = ctx.contextPct;
        if (pct == null) return null;
        const color = colorForPct(pct, ctx.cfg.thresholds, ctx.palette);
        const mode = ctx.cfg.show?.ctxLabel ?? 'text';
        const prefix = mode === 'icon' ? ctx.palette.dim(`${ctx.icons.ctx} `)
                     : mode === 'text' ? ctx.palette.dim('context ')
                     : '';
        return `${prefix}${color(`${pct}%`)}`;
    },
    ...
    dir(ctx) {
        ...
        // 1.5.0: zap prefix removed. CC 2.x renders its own chrome for
        // `--dangerously-skip-permissions`, so our ⚡ next to the dir was
        // redundant. `show.zap` config key is ignored (dropped in normalizeConfig).
        let out = ctx.palette.cyan(name);
        if (ctx.cfg.show.branch) {
            const { branch, dirty } = gitState(cwd);
            if (branch) {
                // 1.5.0: `branchStyle` and `branchMaxLen` dropped. Always parens,
                // no truncation. Users pin a branch-name budget via segment removal.
                const dirtyMark = (dirty && ctx.cfg.show.dirty) ? ctx.palette.red('*') : '';
                const body = `${ctx.palette.green('(')}${ctx.palette.green(branch)}${dirtyMark}${ctx.palette.green(')')}`;
                out += ` ${body}`;
            }
        }
    ...
    //
    // Right-side badge: optional CC update notice + cumulative token count.
    // Both are dim-grey so they don't compete with the bar itself.
    //
    // Context-bar is always visible when the segment is listed. 1.5.0 dropped
    // the `show.bars` master toggle; users remove the segment from the list
    // if they don't want the wide fuel-gauge row.
    'context-bar'(ctx) {
        ...
        let out = `${ctx.palette.white('context')} ${filledColor(filledStr)}${ctx.palette.dim(emptyStr)}`;
        // Update notice and token badge are tag-wrapped so applyLayout's drop
        // ladder can strip them on narrow terminals before the line overflows.
        if (ctx.ccUpdate && ctx.cfg.show.ccUpdate !== false) {
            out += tagPart('ccUpdate', ` ${ctx.palette.dim('update avail')} ${ctx.palette.yellow(ctx.ccUpdate)}`);
        }
        if (ctx.cfg.show.contextTokens !== false) {
            const totalTok = computeContextTokens(ctx.input);
            if (totalTok > 0) out += tagPart('contextTokens', ` ${ctx.palette.dim(fmtKTokens(totalTok))}`);
        }
        return out;
    },

    // Pace explainer line — transient teaching row, see renderPaceExplainer.
    'pace-explainer'(ctx) {
        return renderPaceExplainer(ctx);
    },
    ...
    // 1.5.0: precision hard-coded to 2 decimals (was `costPrecision` config).
    cost(ctx) {
    ...
    // 1.5.0: raw integers only — `compactNumbers` config dropped.
    lines(ctx) {
...
// When a user lists BOTH the compact form and the full form of the same
// rate-limit window (e.g. `5h` and `rate-5h-full`), render only one. The
// full form wins — it's more informative. The compact form still works on
// its own. 1.5.0 addition: fixes a duplicate-row bug the user hit by
// toggling both in the wizard.
const RATE_DEDUP_PAIRS = Object.freeze({
    '5h': 'rate-5h-full',
    '7d': 'rate-7d-full',
});
...
export function renderLine(ctx) {
    ...
    // Rate-limit dedup: when both compact (`5h`) and full (`rate-5h-full`)
    // are listed, keep only the full form. See RATE_DEDUP_PAIRS comment.
    let segments = rawSegments.slice();
    for (const [compact, full] of Object.entries(RATE_DEDUP_PAIRS)) {
        if (segments.includes(compact) && segments.includes(full)) {
            segments = segments.filter(s => s !== compact);
        }
    }
    segments = segments.filter(name => (SEGMENT_ALIASES[name] || name) !== 'worktree');
