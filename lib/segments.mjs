...
export const SEGMENTS = {
    ...
    ctx(ctx) {
        const pct = readContextPct(ctx.input);
    ...
    // Wall-clock duration since session started. Was previously keyed off
    // input.session.start_time, which doesn't exist in the spec — the field
    // is cost.total_duration_ms. Renamed to `elapsed`; `session` kept as an
    // alias for back-compat with any configs that still name it.
    elapsed(ctx) {
