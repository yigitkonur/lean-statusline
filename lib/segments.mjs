...
export const SEGMENTS = {
    ...
    'context-bar'(ctx) {
        ...
        // Cumulative token badge — always rounded to nearest K, no decimals.
        if (ctx.cfg.show.contextTokens !== false) {
            const totalTok = (probe('context_window.total_input_tokens', ctx.input) ?? 0)
                + (probe('context_window.total_output_tokens', ctx.input) ?? 0);
