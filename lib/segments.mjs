...
export const SEGMENTS = {
    // Silent on local sessions. When SSH_CONNECTION is set, show the server
    // IP with a lock icon so it's obvious at a glance the agent is running
    // on a remote box. Users can override with LEAN_STATUSLINE_SSH_HOST
    // to surface a friendlier name.
    ssh(ctx) {
        const override = process.env.LEAN_STATUSLINE_SSH_HOST;
        const host = override || detectSshHost();
        if (!host) return null;
