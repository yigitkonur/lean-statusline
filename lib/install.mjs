...
//      If our own bin lives under that tree, we're fresh from npx.
//   2. npm_config_user_agent — npm/npx set this to something like
//      "npm/10.x.x node/v20.x.x ... npx/..." when the invocation
//      came from `npx`. Survives npx-delegation-to-global (which
//      otherwise makes the bin path look like a normal global install).
