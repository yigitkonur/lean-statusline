...
// Detect whether the install command is being run through `npx`. npm
// stages npx packages under ~/.npm/_npx/<hash>/... — if our own bin
// lives under that tree, we know we're in an npx invocation and should
// prefer an npx-flavored statusLine command (self-updating, no global
// install required).
export function isRunningViaNpx(binPath) {
    return /[\\/](_npx|[.]npm[\\/]_npx)[\\/]/.test(binPath);
}
