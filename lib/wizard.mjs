...
async function askPreset(cfg) {
    ...
    while (true) {
        ...
        for (const [idx, name] of PRESET_NAMES.entries()) {
            ...
            // Name + one-line tagline on a single row — tight, readable.
            process.stdout.write(`${BOLD}(${idx + 1}) ${p.name.padEnd(8)}${RESET}${DIM}${p.description}${RESET}\n`);
            // Live preview of this preset rendered with the user's tty settings.
        ...
        // Compact footer: single-key prompt + ssh guarantee footnote. No
        // redundant menu re-listing the options — inline numbers above are
        // enough. Only (q) needs reminding since it's not next to any preset.
        process.stdout.write(`${DIM}↳ 🔒 ssh host always shown on remote, every preset.${RESET}\n\n`);
        process.stdout.write(`${DIM}press 1–${PRESET_NAMES.length} or q${RESET} › `);
        const k = (await readKey()).toLowerCase();
        if (k === 'q') process.exit(0);
        const idx = parseInt(k, 10) - 1;
        if (Number.isInteger(idx) && idx >= 0 && idx < PRESET_NAMES.length) {
            return applyPreset(current, PRESET_NAMES[idx]);
        }
        // Unrecognized key — loop and redraw.
    }
}
