    ...
    try {
        while (true) {
            ...
            if (currentPage !== 5) {
                ...
                if (condNote) {
                    out += `${DIM}${condNote}${RESET}\n`;
                    condNoteLines = 1;
                }
            }
            // 1.5.4: pin preview block height so cycling presets / palettes
            // doesn't shift the form field up and down. `full` is the tallest
            // (3 body rows + 1 note row = 4); pad shorter previews with blank
            // lines up to the same budget. Page 5 already compresses to one
            // line and doesn't need padding — the form there is the last thing
            // below anyway.
            if (currentPage !== 5) {
                const FIXED_CONTENT_ROWS = 4;
                const bodyRows = previewBody.split('\n').length;
                const padRows = Math.max(0, FIXED_CONTENT_ROWS - bodyRows - condNoteLines);
                if (padRows > 0) out += '\n'.repeat(padRows);
            }
