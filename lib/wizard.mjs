...
// Force `current` and `weekly` rate bars onto separate preview lines so both
// stay visible at any rateBarWidth. The real preset keeps them on one line
// to save vertical space; the preview prefers clarity. Idempotent: skipped
// if either is missing or a `\n` already sits between them.
function splitRateBarLine(segments) {
    const idx5 = segments.indexOf('rate-5h-full');
    const idx7 = segments.indexOf('rate-7d-full');
    if (idx5 < 0 || idx7 < 0) return segments;
    const [lo, hi] = idx5 < idx7 ? [idx5, idx7] : [idx7, idx5];
    for (let i = lo + 1; i < hi; i++) {
        if (segments[i] === '\n') return segments;
    }
    return [...segments.slice(0, hi), '\n', ...segments.slice(hi)];
}
...
function renderPreview(state) {
    ...
    //   - separate rate-bar rows so `weekly` never hides behind overflow
    ...
    //     overflow visibly with a trailing `…`).
    const sshEnabled = (state.segments || []).includes('ssh');
    const previewCfg = {
        ...state,
        segments: splitRateBarLine(state.segments || []),
